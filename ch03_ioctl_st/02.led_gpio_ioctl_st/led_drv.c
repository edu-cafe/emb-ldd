...
long led_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if(_IOC_NR(cmd) > LED_MAXNR) return -EINVAL;
	...
	case LED_GETSTATE :
		...
		break;
	case LED_READ :
		printk("[led_DD] ioctl => LED_READ..\n");
		.............
		break;
	case LED_WRITE :
		printk("[led_DD] ioctl => LED_WRITE(size:%d, msg:%d)..\n", info->size, info->buff[0]);
		.............

		break;
	case LED_RW :
		printk("[led_DD] ioctl => LED_RW..\n");
		break;
	}
	return 0;
}
